//! Data model for the Dataset Intelligence feature: analysis requests, the
//! persisted report, and the background-job progress envelope.
//!
//! Everything serializes `camelCase` to match the frontend `types/core.ts`
//! convention. The full [`AnalysisReport`] is stored as a JSON blob in the
//! `analysis_reports` table; a lighter [`ReportSummary`] is returned when
//! listing past reports so the dashboard doesn't have to hydrate megabytes.

use serde::{Deserialize, Serialize};

// ── Request / configuration ────────────────────────────────────────────────

/// Tunable thresholds for the analysis. All optional — [`AnalysisConfig::resolved`]
/// fills in defaults so the frontend can send a partial object (or nothing).
