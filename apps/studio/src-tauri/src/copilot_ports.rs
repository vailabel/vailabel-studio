...
//! events) plus a cloned `AppHandle` and the entity store. These are the only
//! places that know both the copilot module and the inference/LLM internals.
...
use crate::ai::llm;
use crate::ai::model::{PipelineRunPayload, PredictionGeneratePayload};
use crate::ai::plugin::{self, PromptInput};
...
impl BinaryCopilotInference {
    pub fn new(ai: Arc<AiService>, store: Arc<dyn EntityStore>, app: tauri::AppHandle) -> Self {
        Self { ai, store, app }
    }
}
...
impl CopilotInference for BinaryCopilotInference {
    fn image(&self, image_id: &str) -> DomainResult<Option<Value>> {
        self.store.get_entity("images", image_id).map_err(repo_err)
    }
