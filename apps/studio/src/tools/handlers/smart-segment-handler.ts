...
export class SmartSegmentHandler implements ToolHandler {
  ...
  /** Typed view over the loosely-typed shared tool state. */
  private get state() {
    return this.context.toolState as {
      isDragging?: boolean
  ...
  onMouseUp() {
    const { isDragging, startPoint, currentPoint } = this.state
  ...
  getUIState(): SmartSegmentHandlerUIState {
    return {
      isDragging: (this.state.isDragging ?? false) as boolean,
      tempAnnotation:
        (this.context.toolState.tempAnnotation as Partial<Annotation>) ||
        undefined,
    }
  }
}
