...
import { calculatePolygonCentroid } from "@/features/studio/model/lib/canvas-utils"
...
export class MoveHandler implements ToolHandler {
  ...
  getUIState(): MoveHandlerUIState {
    const getMovingCoordinates = (): Point[] | null => {
    ...
    const getResizingCoordinates = (): Point[] | null => {
    ...
    return {
      ...
      movingAnnotationId: this.context.toolState.movingAnnotationId ?? null,
      resizingAnnotationId: this.context.toolState.resizingAnnotationId ?? null,
      previewCoordinates: this.context.toolState.previewCoordinates ?? null,
